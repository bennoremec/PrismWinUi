...
{
    private readonly ModuleDependencySolver solver;
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => { solver.AddModule(null); });
    }

    [Fact]
    public void CannotAddEmptyModuleName()
    {
        var ex = Assert.Throws<ArgumentException>(() => { solver.AddModule(string.Empty); });
    }

    [Fact]
    public void CannotAddDependencyWithoutAddingModule()
    {
        var ex = Assert.Throws<ArgumentException>(() => { solver.AddDependency("ModuleA", "ModuleB"); });
    }
